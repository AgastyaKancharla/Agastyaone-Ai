/**
 * GST state codes. The first two digits of a GSTIN, and the value that decides
 * CGST+SGST (intra-state) versus IGST (inter-state) on every invoice you issue.
 * Getting it wrong is a tax error, not a display error, so it is picked from a
...
export const stateName = (code?: string | null) =>
  GST_STATES.find((s) => s.code === code)?.name ?? null;
